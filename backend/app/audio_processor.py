...
#         """
#         Optimized pattern detection with faster processing
#         """
...
#             target_sr = 22050  # Half of 44100 for faster processing
...
#             correlation = self.fft_correlate(target_audio, pattern_audio)
#             correlation_norm = correlation / (np.max(np.abs(correlation)) + 1e-8)
            
#             print("📊 Finding peaks...")
#             # Find peaks in correlation
#             min_distance = max(1, int(len(pattern_audio) * 0.3))
#             peaks, properties = find_peaks(
#                 correlation_norm,
#                 height=0.2,  # Slightly higher threshold for better accuracy
#                 prominence=0.15,
...
#             detection_times = peaks / target_sr
            
#             # Filter valid detections
#             max_time = len(target_audio) / target_sr
...
#             detections = [
#                 {"time": float(t), "confidence": float(v)}
...
#             downsample_factor_waveform = max(1, len(target_audio) // 500)  # Reduced from 800 to 500
#             downsample_factor_corr = max(1, len(correlation_norm) // 500)
            
#             waveform_viz = target_audio[::downsample_factor_waveform].tolist()
#             correlation_viz = correlation_norm[::downsample_factor_corr].tolist()
...
#                 "pattern_duration": float(len(pattern_audio) / pattern_sr),
#                 "target_duration": float(len(target_audio) / target_sr),
#                 "sample_rate": int(target_sr),
#                 "waveform_data": waveform_viz,
#                 "correlation_data": correlation_viz
#             }
...
#         # Pad pattern to match target length
